...
class IntelPowerGadget(NameMixin):
    ...
    def get_exe_default_path(self):
        if self.platform == 'darwin':
            ...
            exe_path = '/home/sparky/Downloads/power_gadget/power_gadget'
        else:
            raise ValueError('{}: {} platform currently not supported'.format(self.name, self.platform))
        return exe_path
        ...
        if 'linux' in self.platform:
            with open(output_file_path, 'w+') as f:
                subprocess.check_call(
                    ['sudo', exe_file_path, '-d', str(duration), '-e', str(self.sampling_rate)],
                    stdout=f
                )
        else:
