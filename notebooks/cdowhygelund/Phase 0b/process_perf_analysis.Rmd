...
for (exp_name in exps){
  plot(plot_perf_fits(perf_agg[[exp]], exp_name = exp_name))
}
```
...
```{r, fig.width = 12, fig.height = 8}
nyt_child_thread = child_threads[['exp_nytimes']]

p_names <- c('', 'Chrome_ChildThread', 'Socket Thread', 'JS Helper', 'DOM Worker', 'MediaPlayback #2')
...
```{r}
ggplot(f_m1_cpuUser[f_m1_cpuUser$drain < 90, ], aes(sum, drain))+
  geom_point(aes(color=exp)) + 
  geom_smooth(method = "lm", se=TRUE, formula = y ~ x)+
  guides(color=FALSE)
```
...
plot(f_m1_cpuKernel$sum, f_m1_cpuKernel$drain)
```

```{r}
features_m1 %>%
  group_by(run_id, exp) %>%
  summarise(sum = sum(sum)) -> features_m1_sum

plot(features_m1_sum$sum, features_m1_sum$drain)
