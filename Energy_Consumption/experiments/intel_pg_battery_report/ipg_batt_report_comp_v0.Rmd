...
results$ratio <- results$ChargeCapacity/results$FullChargeCapacity
...
The timestamps is converted to the number of minutes since the experiment started. This enables easy alignment with the Intel Power Gadget data.

```{r batt_repo_clean}
results$dates <- parse_date_time(results$Timestamp, 'YmdHMS')
results$minutes <- (hour(results$dates)-7)*60+minute(results$dates)
head(results[, c('Iteration', 'minutes', 'ChargeCapacity', 'ratio')], n=10)
```

Updating/resolution of Battery Report appears to be larger than one minute, as can be seen by repeated `ratio` values through time.
...
Compare the battery report `ratio` with the Intel Power Gadget cumulative processor energy

```{r final_plot_1, echo=FALSE}
ggplot(final, aes(ratio, Cumulative.Processor.Energy_0.mWh.)) +
  geom_point()+
...
ggplot(final.cost, aes(ratio, Cumulative.Processor.Energy_0.mWh.)) +
  geom_point()+
  geom_smooth(method='lm')
```
```{r final_comp}
lm(ratio ~ Cumulative.Processor.Energy_0.mWh., final.cost)
```
## Next Steps
